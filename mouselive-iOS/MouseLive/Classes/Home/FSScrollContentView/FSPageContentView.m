...
@property (nonatomic, weak) UIView *parentVC;//父视图
...
- (instancetype)initWithFrame:(CGRect)frame childVCs:(NSArray *)childVCs parentVC:(UIView *)parentVC delegate:(id<FSPageContentViewDelegate>)delegate
{
...
- (void)setupSubViews
{
    ...
    for (UIView *childVC in self.childsVCs) {
        [self.parentVC addSubview:childVC];
    }
...
{
    [cell.contentView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    UIView *childVc = self.childsVCs[indexPath.row];
