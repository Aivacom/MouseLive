...
- (void)initWithProperty
{
    self.itemMargin = 20;
    self.selectIndex = 0;
    self.titleNormalColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
    self.titleSelectColor = [UIColor blackColor];
    self.titleFont = [UIFont fontWithName:FONT_Light size:14.0];
    self.indicatorColor = [UIColor sl_colorWithHexString:@"#0DBE9E"];
    ...
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (self.indicatorType) {
            case FSIndicatorTypeDefault:
                self.indicatorView.frame = CGRectMake(selectBtn.frame.origin.x , CGRectGetHeight(self.scrollView.bounds) - 4, CGRectGetWidth(selectBtn.bounds), 4);
                break;
            ...
            case FSIndicatorTypeCustom:
                ...
                self.indicatorView.bounds = CGRectMake(0, 0, 24.0, 4);
                break;
